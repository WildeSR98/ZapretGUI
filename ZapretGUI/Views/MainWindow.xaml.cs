...
public partial class MainWindow : Window
{
    ...
    private static MainViewModel CreateDesignTimeViewModel()
    {
        ...
        var registry = new AiStrategyRegistry(Path.Combine(dir, "ZapretGUI-ai-strategies.json"));
        registry.Load();
        var history = new AiHistoryStore(Path.Combine(dir, "ZapretGUI-ai-history.jsonl"));
    ...
    {
        _trayIcon.ShowBalloon("ZapretGUI", $"Профиль переключён: {profileName}");
        _trayIcon.UpdateTooltip($"ZapretGUI — {profileName}");
    ...
    protected override void OnStateChanged(EventArgs e)
    {
        ...
        if (WindowState == WindowState.Minimized)
        {
            ShowInTaskbar = false;
            Hide();
            _trayIcon.ShowBalloon("ZapretGUI", "Приложение свёрнуто в трей");
        }
    }
    ...
    protected override void OnClosing(CancelEventArgs e)
    {
        ...
        if (!_isClosingConfirmed)
        {
            ...
            {
                _isClosingConfirmed = true;
                _logger?.LogInformation("User confirmed ZapretGUI shutdown from main window.");
