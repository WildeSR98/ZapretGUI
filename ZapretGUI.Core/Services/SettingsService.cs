...
///
/// ZapretGUI is currently distributed as a portable app, so settings intentionally stay
/// next to ZapretGUI.exe. This service hardens the existing behavior without changing
...
{
    private const string SettingsFileName = "ZapretGUI-settings.json";
    ...
    public void Save(AppSettings settings)
    {
        ...
        {
            Trace.TraceError($"ZapretGUI settings save failed. Path='{SettingsPath}'. Error='{ex}'");
    ...
    private static bool TryLoad(string path, out AppSettings settings)
    {
        ...
        {
            Trace.TraceError($"ZapretGUI settings load failed. Path='{path}'. Error='{ex}'");
            return false;
        }
    }
    ...
    private void TryRestoreBackupAsPrimary()
    {
        try
        {
            ...
            Trace.TraceWarning($"ZapretGUI settings restored from backup. Backup='{BackupPath}', Target='{SettingsPath}'.");
        }
        catch (Exception ex)
        {
            Trace.TraceError($"ZapretGUI settings backup restore failed. Backup='{BackupPath}', Target='{SettingsPath}'. Error='{ex}'");
        }
    }
