...
public sealed partial class UpdaterService : IUpdaterService
{
    ...
    public string GetLocalVersion(string engineDir)
    {
        ...
        // Fallback: LOCAL_VERSION из service.bat (до первого обновления через ZapretGUI)
    ...
    {
        var tempZip     = Path.Combine(Path.GetTempPath(), "ZapretGUI_update.zip");
        var tempExtract = Path.Combine(Path.GetTempPath(), "ZapretGUI_update_extract");
