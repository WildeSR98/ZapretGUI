...
namespace ZapretGUI.Updater.Services;

/// <summary>Информация о доступном обновлении самого приложения ZapretGUI.</summary>
...
public interface IAppUpdaterService
{
    ...
    /// <summary>Проверяет наличие нового релиза ZapretGUI через редирект GitHub Releases (без API, без лимитов).</summary>
...
public sealed class AppUpdaterService : IAppUpdaterService
{
    ...
    private const string ReleasesAtomUrl =
        "https://github.com/klondike0x/ZapretGUI/releases.atom";

    // Имя asset в релизе: ZapretGUI-v1.4.1-portable.zip
    private const string AssetNameTemplate = "ZapretGUI-{0}-portable.zip";

    // Прямой URL скачивания (не API, CDN GitHub — лимитов нет)
    private const string DownloadUrlTemplate =
        "https://github.com/klondike0x/ZapretGUI/releases/download/{0}/ZapretGUI-{0}-portable.zip";

    private const string UserAgent = "ZapretGUI-AppUpdater";
        ...
        CancellationToken ct = default)
    {
        ...
        var tempZip  = Path.Combine(Path.GetTempPath(), $"ZapretGUI_{update.Version}.zip");
        var tempDir  = Path.Combine(Path.GetTempPath(), $"ZapretGUI_{update.Version}_extracted");
        var batPath  = Path.Combine(Path.GetTempPath(), "_ZapretGUI_updater.bat");

        try
        {
            // ── 1. Скачиваем zip ──────────────────────────────────────────
            onProgress($"⬇️ Скачиваем ZapretGUI v{update.Version}...");
            ...
            // Ищем ZapretGUI.exe внутри архива (может быть в подпапке)
            var extractedExe = Directory.GetFiles(tempDir, "ZapretGUI.exe", SearchOption.AllDirectories)
                                         .FirstOrDefault();
            if (extractedExe is null)
                return (false, "ZapretGUI.exe не найден внутри архива");
            ...
            // Папка внутри архива, где лежит ZapretGUI.exe — копируем ВСЁ из неё
            var extractedSourceDir = Path.GetDirectoryName(extractedExe)!;

            var bat = $"""
                @echo off
                chcp 65001 > nul
                echo [ZapretGUI Updater] Ожидаем завершения процесса PID {pid}...
                :waitloop
                ...
                echo [ZapretGUI Updater] Устанавливаем v{update.Version}...
                xcopy /E /Y /I "{extractedSourceDir}\*" "{exeDir}\"
                if errorlevel 1 (
                    echo [ZapretGUI Updater] Ошибка копирования!
                    pause
                    exit /b 1
                )
                echo [ZapretGUI Updater] Завершаем дочерние процессы...
                ...
                echo [ZapretGUI Updater] Очищаем временные файлы...
                del /F /Q "{tempZip}" > nul 2>&1
                rd /S /Q "{tempDir}" > nul 2>&1
                echo [ZapretGUI Updater] Запускаем ZapretGUI v{update.Version}...
                start "" "{newExePath}"
                del "%~f0"
                """;
